...
//       console.log('⚠️ Missing fields in emotion data');
//       return res.status(400).json({ message: 'All fields are required' });
//     }

//     const child = await Child.findOne({ userId });
//     if (!child) {
//       console.log('❌ Child not found for userId:', userId);
//       return res.status(404).json({ message: 'Child not found' });
//     }

//     child.emotionHistory.push({ emotion, question, timestamp: new Date() });
//     await child.save();
//     console.log('✅ Emotion saved for userId:', userId, 'Emotion:', emotion, 'Question:', question);
...
//     io.emit('gameReportUpdate', { userId, score, emotions, question, isCorrect, completedAt: new Date() });
